...
func IntDeclare(tokenReader *TokenReader) (*ASTNode, error) {
	...
	if tokenType == nil || tokenType.TokenType() != Int {
	...
	// 解析 Assignment=
	tokenType = tokenReader.Peek()
	if tokenType == nil || tokenType.TokenType() != Assignment {
...
func AdditiveLoop(tokenReader *TokenReader) (*ASTNode, error) {
	...
	for {
		// 应用 add' 规则
		tokenType := tokenReader.Peek()
		if tokenType != nil && (tokenType.TokenType() == Plus || tokenType.TokenType() == Minus) {
...
func MultiplicativeLoop(tokenReader *TokenReader) (*ASTNode, error) {
	...
	for {
		tokenType := tokenReader.Peek()
		if tokenType != nil && (tokenType.TokenType() == Star || tokenType.TokenType() == Slash) {
			tokenType = tokenReader.Read()
...
func Primary(tokenReader *TokenReader) (*ASTNode, error) {
	...
	if tokenType.TokenType() == IntLiteral {
		...
		tokenType = tokenReader.Peek()
		if tokenType != nil && tokenType.TokenType() == RightParen {
			tokenType = tokenReader.Read()
		} else {
